...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/mcp_sentinel/policy/loader.py"
      provides: "Load + validate policy"
  key_links:
    - from: "src/mcp_sentinel/policy/loader.py"
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/policy/models.py
src/mcp_sentinel/policy/__init__.py</files>
  <action>
...
- Keep schema predictable; avoid complex match expressions.
  ...
  <verify>uv run python -c "from mcp_sentinel.policy.models import Policy; print('ok')"</verify>
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/policy/loader.py
...
- Provide example rate limits (conservative defaults).
  ...
  <verify>uv run python -c "from mcp_sentinel.policy.loader import load_policy; load_policy('config/policies.yaml'); print('loaded')"</verify>
...
<success_criteria>
- A validated policy is required to start Sentinel, and the default policy denies access unless explicitly allowed.
</success_criteria>
