...
must_haves:
  ...
  artifacts:
    - path: "src/mcp_sentinel/middleware/authz.py"
  ...
  key_links:
    - from: "src/mcp_sentinel/proxy/sentinel_server.py"
      to: "src/mcp_sentinel/middleware/authz.py"
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/middleware/authz.py</files>
  <action>
...
- Load policy at startup (single load; not per-request). Support selecting the policy path via env var:
  - Read `SENTINEL_POLICY_PATH` (fallback to `config/policies.yaml`).
...
<success_criteria>
- Sentinel enforces tool-level authorization according to policy and roles.
</success_criteria>
