...
| --- | --- | --- | --- |
| 1 | Every tool request can be recorded with timestamp and client identity | ✓ VERIFIED | `AuditEvent` includes `ts` and `api_key_id` (`src/mcp_sentinel/audit/models.py:34`, `src/mcp_sentinel/audit/models.py:35`); logger persists to `audit_events` (`src/mcp_sentinel/audit/logger.py:10`, `src/mcp_sentinel/audit/logger.py:59`); tests pass for insert path (`tests/test_audit_logger.py:50`, `tests/test_audit_logger.py:73`). |
| 2 | Every tool call attempt produces an audit event (allowed or denied) | ✓ VERIFIED | Proxy writes audit event in `finally` after allow/deny/error outcomes (`src/mcp_sentinel/proxy/sentinel_server.py:106`, `src/mcp_sentinel/proxy/sentinel_server.py:244`); integration test proves 401/403/200 rows exist (`tests/test_audit_integration.py:209`, `tests/test_audit_integration.py:259`). |
| 3 | Operator can view recent audit events in a simple dashboard | ✓ VERIFIED | Dashboard routes and DB query exist (`src/mcp_sentinel/ui/dashboard.py:49`, `src/mcp_sentinel/ui/dashboard.py:172`, `src/mcp_sentinel/ui/dashboard.py:209`), CLI wiring exists (`src/mcp_sentinel/main.py:32`, `src/mcp_sentinel/main.py:102`), and runtime dashboard/access/filter checks were human-approved on 2026-02-14. |
...
| --- | --- | --- | --- |
| `src/mcp_sentinel/audit/models.py` | Audit event schema with bounded payloads | ✓ VERIFIED | Exists (218 lines), substantive validators and serialization (`src/mcp_sentinel/audit/models.py:31`, `src/mcp_sentinel/audit/models.py:79`, `src/mcp_sentinel/audit/models.py:84`), wired via imports in logger/proxy. |
| `src/mcp_sentinel/audit/logger.py` | Audit event writer to Postgres | ✓ VERIFIED | Exists (85 lines), `INSERT INTO audit_events` statement and execution path (`src/mcp_sentinel/audit/logger.py:10`, `src/mcp_sentinel/audit/logger.py:59`), wired from proxy (`src/mcp_sentinel/proxy/sentinel_server.py:14`, `src/mcp_sentinel/proxy/sentinel_server.py:244`). |
...
| `src/mcp_sentinel/ui/dashboard.py` | HTTP page/API for recent audit events | ✓ VERIFIED | Exists (418 lines), exposes `/dashboard` and `/api/events`, authenticates admin key, queries `audit_events` with filters (`src/mcp_sentinel/ui/dashboard.py:51`, `src/mcp_sentinel/ui/dashboard.py:120`, `src/mcp_sentinel/ui/dashboard.py:197`). Runtime UX still needs human check. |

### Key Link Verification

| From | To | Via | Status | Details |
| --- | --- | --- | --- | --- |
| `src/mcp_sentinel/audit/logger.py` | `src/mcp_sentinel/storage/postgres.py` | `INSERT audit_events` | WIRED | Logger resolves pool with `get_postgres_pool()` and executes SQL insert into `audit_events` (`src/mcp_sentinel/audit/logger.py:44`, `src/mcp_sentinel/audit/logger.py:10`, `src/mcp_sentinel/audit/logger.py:59`). |
| `src/mcp_sentinel/proxy/sentinel_server.py` | `src/mcp_sentinel/audit/logger.py` | log after decision | WIRED | Proxy injects/creates `AuditLogger` and always calls `_write_audit_event(...)` in `finally`, which calls `audit_logger.log_event(event)` (`src/mcp_sentinel/proxy/sentinel_server.py:37`, `src/mcp_sentinel/proxy/sentinel_server.py:106`, `src/mcp_sentinel/proxy/sentinel_server.py:244`). |
| `src/mcp_sentinel/ui/dashboard.py` | `src/mcp_sentinel/storage/postgres.py` | `SELECT from audit_events` | WIRED | Dashboard fetch path calls `get_postgres_pool()`, executes `FROM audit_events` query, and returns rows to HTML/JSON response builders (`src/mcp_sentinel/ui/dashboard.py:177`, `src/mcp_sentinel/ui/dashboard.py:209`, `src/mcp_sentinel/ui/dashboard.py:95`). |
...
| AUDIT-02: Log client identity (API key) | ✓ SATISFIED | Identity fields present in model/logger and included in decision payload (`src/mcp_sentinel/audit/models.py:35`, `src/mcp_sentinel/proxy/sentinel_server.py:65`). |
| AUDIT-03: Log tool name and parameters | ✓ SATISFIED | Tool name and request arguments captured in audit event (`src/mcp_sentinel/proxy/sentinel_server.py:114`, `src/mcp_sentinel/proxy/sentinel_server.py:202`). |
...
**Test:** Start dashboard (`mcp-sentinel dashboard`) with populated audit data, then open `/dashboard` using a valid admin API key.
