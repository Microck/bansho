...
must_haves:
  ...
  artifacts:
    - path: "src/mcp_sentinel/audit/logger.py"
      provides: "Audit event writer to Postgres"
    - path: "src/mcp_sentinel/audit/models.py"
      provides: "Audit event schema"
  key_links:
    - from: "src/mcp_sentinel/audit/logger.py"
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/audit/models.py
src/mcp_sentinel/audit/__init__.py</files>
  <action>
...
- Keep JSON payloads bounded (truncate large blobs to safe size; store only what is needed for demo).
  ...
  <verify>uv run python -c "from mcp_sentinel.audit.models import AuditEvent; print('ok')"</verify>
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/audit/logger.py
