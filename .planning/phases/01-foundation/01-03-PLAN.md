...
must_haves:
  truths:
    - "A client can connect to Bansho and list tools from the upstream server"
    - "A client tool call is forwarded to the upstream server and returns the same result"
    - "A client can list/read resources and list/get prompts through Bansho when the upstream supports them"
  artifacts:
    - path: "src/bansho/proxy/bansho_server.py"
      provides: "Bansho acts as MCP server to clients"
    - path: "src/bansho/proxy/upstream.py"
  ...
  key_links:
    - from: "src/bansho/proxy/bansho_server.py"
      to: "src/bansho/proxy/upstream.py"
      via: "forward MCP request/response across method families"
      pattern: "(tools|resources|prompts)/(list|call|read|get)"
---

<objective>
Build a fully spec-compliant MCP passthrough where Bansho presents an MCP server to clients and forwards requests to a configured upstream MCP server.
...
<task type="auto">
  ...
  <files>src/bansho/proxy/upstream.py
src/bansho/proxy/__init__.py</files>
  <action>
...
- Expose async methods that Bansho will use:
  - session lifecycle: `initialize()` (so Bansho can reflect upstream capabilities)
...
uv run python -c "from bansho.proxy.upstream import UpstreamConnector; print(UpstreamConnector)"
  </verify>
  <done>Upstream connector module exists with a stable API surface for tool listing and calling.</done>
</task>

<task type="auto">
  <name>Task 2: Bansho MCP server that forwards tool methods</name>
  <files>src/bansho/proxy/bansho_server.py
src/bansho/main.py</files>
  <action>
Implement Bansho as an MCP server to clients:
...
- Add a minimal health/log line on startup showing listen addr and upstream config.
  ...
  <verify>uv run python -m bansho</verify>
...
- Start Bansho configured to connect to that upstream.
- Assert through Bansho:
...
<success_criteria>
- Bansho can proxy an MCP tool listing and tool call to an upstream server without modifying payloads.
</success_criteria>
