...
- Cleanup expectations:
  - Always tear down any processes started by this script (vulnerable server, bansho process) via `trap` on EXIT.
...
- Starts Bansho as an MCP stdio server, configured as a proxy to the vulnerable server (stdio upstream):
  - Pin the exact bansho invocation used by the script:
    - command: `uv`
    - args: `run bansho serve`
  - Provide env to the Bansho process (and print them once at start for debugging):
...
- Demonstrates the after-state enforcement (assert and print each outcome):
  ...
  Implement these calls by talking to Bansho over stdio using the same pattern as `tests/test_passthrough.py`:
