...
must_haves:
  ...
  artifacts:
    - path: "src/mcp_sentinel/cli/keys.py"
      provides: "CLI for api-key create/revoke/list"
  key_links:
    - from: "src/mcp_sentinel/cli/keys.py"
      to: "src/mcp_sentinel/auth/api_keys.py"
      via: "calls repository functions"
      pattern: "from mcp_sentinel\\.auth\\.api_keys import"
---
...
Output: `mcp-sentinel keys ...` subcommands.
</objective>
...
<task type="auto">
  ...
  <files>src/mcp_sentinel/cli/keys.py
src/mcp_sentinel/cli/__init__.py</files>
  <action>
Add a simple CLI:
...
- Ensure the default role is `readonly`.
  ...
  <verify>uv run python -m mcp_sentinel keys --help</verify>
...
- `mcp_sentinel serve` starts the proxy.
- `mcp_sentinel keys ...` runs key management.
...
<verification>
- `uv run python -m mcp_sentinel keys create --role readonly` creates a key and prints it once
- `uv run python -m mcp_sentinel keys list` shows the created key id
</verification>
